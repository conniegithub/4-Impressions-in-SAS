...
*SUBSETTING, CONVERTING CHAR TO NUM AND RENAME, REMOVE MISSING DATA IN VISIT DATA;
...
%macro combine(new, data1, data2, data3);
	...
	data &new;
		set temp;
		if gender = 'M' then genderN = 1;
		else if gender = 'F' then genderN = 0;
		if Inet_owner = 'y' then Inet_ownerN = 1;
		else if Inet_owner = 'n' then Inet_ownerN = 0;
...
/*
*check dates are indeed sas numeric dates;
data new;
	set Sheet;
	n = visit_dt - 1;
	d = put(n, mmddyy10.);
...
proc glm data = Sheet plots = all;
	model response = visitor_ID visit_dt age Yr_income gender Inet_owner;
run;
